...
inquiry <- "MINISTRY OF DEFENCE: MAJOR PROJECTS REPORT 1998"
publication_reference <- "HC 00209"

inquiry_link <- "https://publications.parliament.uk/pa/cm199900/cmselect/cmpubacc/247/24702.htm"
publication_link <- "https://publications.parliament.uk/pa/cm199900/cmselect/cmpubacc/247/0020901.htm"

members_present <- 
  paste("Mr David Davis, in the Chair",
        "Mr Alan Campbell",	"Mr Geraint Davies",
        "Mr Barry Gardiner",	"Mr Nigel Griffiths",
        "Mr Jim Murphy	",	"Mr Richard Page",
        "Mr David Rendel", "Mr Gerry Steinberg", 
        "Mr Charles Wardle", sep = "; ")	

witnesses_present <- 
  paste("SIR ROBERT WALMSLEY, KCB, Chief of Defence Procurement, Ministry of Defence", sep = "; ")

publication_title <- "MINUTES OF EVIDENCE TAKEN BEFORE THE SELECT COMMITTEE OF PUBLIC ACCOUNTS"
publication_date <- "WEDNESDAY 9 FEBRUARY 2000"
...
src <- purrr::map(pages, function(p){
  ...
  url <- stringr::str_sub(publication_link, 1, nchar(publication_link)-6) %>%
    paste0("{p}.htm") %>%
    stringr::str_glue()
...
# Save ----
pac_199900_hc00209 <- list(meta = meta, transcript = text)
save(object = pac_199900_hc00209, file = "data/pac_199900_hc00209.rda")
